...
Если область пересекает границу зон Гаусса-Крюгера или зона выходит за диапазон `1..32`, экспорт будет заблокирован. В `v1.4.0` внутри ядра уже подготовлено разбиение расчетной области на зональные сегменты, но пользовательский многозонный экспорт включается отдельным этапом `v1.5.0`; до этого такую область нужно разбить на отдельные проекты по зонам.
...
Для контрольной проверки `v1.4.0` вручную проверьте:
