...
Для контрольной проверки `v2.0.3` вручную проверьте:

- запуск EXE и отображение главного окна;
- пункт меню `Инструменты` -> `Точки из Excel...` и открытие отдельного окна точек;
- загрузку sample-first `.xlsx` в окне `Точки из Excel`, отображение таблицы точек и ошибок по строкам;
- смену общего цвета и прозрачности точек, выбор итогового `.kml` и успешную генерацию point-KML;
