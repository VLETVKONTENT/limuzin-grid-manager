...
LIMUZIN GRID MANAGER — настольное Windows-приложение для подготовки KML-сеток `1000x1000` и `100x100` метров по координатам в системе СК-42 / Пулково-42, Гаусс-Крюгер. Приложение преобразует координаты в WGS84 и формирует KML-файлы, которые можно использовать в AlpineQuest и других инструментах, поддерживающих KML.

Проект ориентирован на практическую работу с прямоугольными участками: пользователь задает две точки области, выбирает состав сетки, схему нумерации, стиль KML и формат экспорта, а приложение показывает предпросмотр, выполняет проверку входных данных и сохраняет готовый результат.
...
6. Настройте KML-стиль: линии, заливку больших квадратов и заливку `100x100`.
...
- автоматического разбиения области по зонам в `v1.0.0` нет;
...
Перед записью приложение показывает сводку: сколько файлов будет создано, сколько KML-объектов будет записано и какой размер результата ожидается примерно.
...
Пользовательские `.lgm.json`, `.kml`, `.kmz` и `.zip` добавлены в `.gitignore`, чтобы случайно не публиковать личные рабочие файлы в репозитории.
...
Для `v1.0.0` проверено:

- `43 passed`;
- компиляция `src` и `tests` без ошибок;
- автоматическое восстановление последнего `.lgm.json` покрыто UI-тестом;
- EXE собран с `FileVersion` и `ProductVersion` `1.0.0.0`;
- smoke-запуск EXE: процесс стартует и не завершается сразу;
- tracked-файлы и история git проверены на отсутствие секретов и личных артефактов.
...
│   ├── core/                     # геометрия, CRS, KML, модели, статистика
│   └── ui/                       # PySide6-интерфейс и предпросмотр
├── tests/                        # автотесты
...
- [`versions/v1.0.0.md`](versions/v1.0.0.md) — релизные заметки текущей версии.
...
Изменения версии описаны в [`versions/v1.0.0.md`](versions/v1.0.0.md).
