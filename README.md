...
**Назначение:** генератор KML- и SVG-сеток

**Координаты:** СК-42 / Пулково-42, Гаусс-Крюгер

LIMUZIN GRID MANAGER — настольное Windows-приложение для подготовки сеток `1000x1000` и `100x100` метров по координатам в системе СК-42 / Пулково-42, Гаусс-Крюгер. Приложение преобразует координаты в WGS84 для KML и формирует SVG-схему в метрах для векторной проверки и редактирования.
...
- Экспорт в один `.svg` с метрическим `viewBox`, слоями `1000x1000` и `100x100`, подписями и техническими `data-*` атрибутами.
...
- автоматического разбиения области по зонам в `v1.1.0` нет;
...
| `.svg` | Один SVG-файл с метрической схемой сетки, слоями и подписями. |
...
Пользовательские `.lgm.json`, `.kml`, `.kmz`, `.zip` и `.svg` добавлены в `.gitignore`, чтобы случайно не публиковать личные рабочие файлы в репозитории.
...
Для `v1.1.0` проверено:

- `49 passed`;
- компиляция `src` и `tests` без ошибок;
- SVG-экспорт, `export_format_id` и вкладка `Экспорт` покрыты автотестами;
- EXE собран с `FileVersion` и `ProductVersion` `1.1.0.0`;
- smoke-запуск EXE: процесс стартует и не завершается сразу;
- стабильные KML/ZIP-сценарии продолжают проходить в автотестах.
...
│   ├── core/                     # геометрия, CRS, KML/SVG, модели, статистика
...
- [`versions/v1.1.0.md`](versions/v1.1.0.md) — заметки текущей версии.
...
Изменения версии описаны в [`versions/v1.1.0.md`](versions/v1.1.0.md).
